...
def test_custom_masking_chars():
    ...
    config = PIIMaskingConfig(
        email_mask_char="#", password_mask_char="*", jwt_mask_char="$"
    )
...
def test_disable_masking():
    ...
    config = PIIMaskingConfig(mask_emails=False, mask_passwords=True, mask_jwt=False)
    ...
    test_data = {
        "user": {
            ...
            "settings": {
                "password": "secret123",
                "tokens": [
                    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.K-4DMpfc_GZt3gU1sY1z_a4Q3d-4-r-4H4g1J8jSwGA"
                ],
            },
        },
        "logs": [
