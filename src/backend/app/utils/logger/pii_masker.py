...
class PIIMaskingConfig:
    ...
    def __init__(
        ...
        self.email_pattern = re.compile(r'[^@\s]+@[^@\s]+\.[^@\s]+')  # Simpler email pattern
        self.jwt_pattern = re.compile(r'[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*')
        self.password_fields = {'password', 'pwd', 'user_password', 'password_reset_token'}
...
def mask_value(value: Any, config: PIIMaskingConfig) -> Any:
    ...
    if isinstance(value, str):
        if config.mask_jwt and (config.jwt_pattern.match(value.replace('Bearer ', '')) or len(value.replace('Bearer ', '')) <= 8):
...
def mask_pii(
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            if return_audit:
                nested_result, nested_audit = mask_pii(value, config, return_audit=True, _audit_log=_audit_log)
        ...
        if isinstance(value, list):
            if config.mask_passwords and is_password_field(key, config):
                ...
                masked_data[key] = [
                    mask_pii(item, config) if isinstance(item, dict)
