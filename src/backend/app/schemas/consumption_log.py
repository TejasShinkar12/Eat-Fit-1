...
class ConsumptionLogBase(BaseModel):
    ...
    inventory_item_id: Optional[uuid.UUID] = Field(
        None,
        description="ID of the inventory item consumed (nullable)",
        json_schema_extra={"example": "b3b7c7e2-8c2a-4e2a-9e2a-123456789abc"},
    )
    item_name: str = Field(
        ...,
        description="Name of the inventory item consumed",
        json_schema_extra={"example": "Greek Yogurt"},
    )
    quantity_consumed: float = Field(
        ..., description="Quantity consumed", json_schema_extra={"example": 1.0}
    )
    calories_consumed: float = Field(
        ..., description="Calories consumed", json_schema_extra={"example": 120.0}
    )
    protein_consumed_g: float = Field(
        ..., description="Protein consumed (g)", json_schema_extra={"example": 10.0}
    )
    carbs_consumed_g: float = Field(
        ..., description="Carbs consumed (g)", json_schema_extra={"example": 15.0}
    )
    fats_consumed_g: float = Field(
        ..., description="Fats consumed (g)", json_schema_extra={"example": 2.0}
    )
    ...
    def non_negative(cls, v, info):
        if v < 0:
            raise ValueError(
                f'{info.field_name.replace("_", " ").capitalize()} must be non-negative'
            )
        return v
...
class ConsumptionLogUpdate(BaseModel):
    ...
    inventory_item_id: Optional[uuid.UUID] = Field(
        None, json_schema_extra={"example": "b3b7c7e2-8c2a-4e2a-9e2a-123456789abc"}
    )
    ...
    calories_consumed: Optional[float] = Field(
        None, description="Calories consumed", json_schema_extra={"example": 120.0}
    )
    protein_consumed_g: Optional[float] = Field(
        None, description="Protein consumed (g)", json_schema_extra={"example": 10.0}
    )
...
class ConsumptionLogSummary(BaseModel):
    """
    Lightweight summary for nested log responses.
    Used for embedding consumption log data within inventory detail responses.
    """
    ...
    item_name: str = Field(
        ...,
        description="Snapshot of the item name at time of consumption",
        json_schema_extra={"example": "Greek Yogurt"},
    )
    quantity_consumed: float = Field(
        ..., description="Quantity consumed", json_schema_extra={"example": 1.0}
    ...
    consumed_at: datetime = Field(
        ...,
        description="Timestamp when the item was consumed",
        json_schema_extra={"example": "2024-06-01T12:00:00Z"},
    )
